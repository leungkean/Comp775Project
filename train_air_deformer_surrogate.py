...
from afa.surrogate.deformer.models import ContinuousDEformer
from afa.utils import add_weight_decay

#os.environ["XLA_PYTHON_CLIENT_PREALLOCATE"] = "false"
tf.config.set_visible_devices([], "GPU")


def load_datasets(dataset, validation_split, batch_size):
    train_ds, ds_info = tfds.load(dataset, split="train", with_info=True)
    val_ds = tfds.load(dataset, split=validation_split)

    data_key = "features" if "features" in ds_info.features else "image"

    if data_key == "image":
        raise NotImplementedError(
            "This script has not yet been configured to work with image data. In the"
            "meantime, you can always reformat the image data to look like tabular"
            "data (e.g. flatten and dequantize)."
        )
...
@click.option(
    ...
    type=click.STRING,
    default="128,256,512",
    help="Comma-separated integers specifying the number of hiddden units in each"
    "layer of the pre-attention MLPs.",
)
@click.option(
    "--index_embedding_dim",
    type=click.INT,
...
def main(
    ...
    run = wandb.init(
        ...
        job_type="train_air_deformer_surrogate",
        mode="disabled" if offline else "online",
        config=config,
    )

    train_ds, val_ds, num_features = load_datasets(
    ...
    state_artifact = wandb.Artifact(
        f"{dataset}_air_deformer", type="air_deformer_model"
    )
